...
<details>
 ...
 - 헤더의 네비에서 취업 과정 클릭시 소개 페이지로 이동. 취업 과정의 메인 페이지 역할
...
- 공개 과정의 과정 상세 페이지와 동일하게 상단 섹션에서 url 공유, 일정 선택, 관심 과정 등록 가능
- 하단 섹션에서는 과정개요, 과정목표, 교육대상, 커리큘럼, 대표강사, 접수안내의 탭메뉴로 이루어진 상세 내용을 확인
...
- 아코디언 메뉴는 reactstrap 라이브러리를 사용했으며 컴포넌트로 분리
- 서브 메뉴 클릭시 해당 메뉴대로 아코디언 내용이 변경
  ...
  - 아코디언 메뉴 하단에 답변에 대한 만족도를 평가하는 별점 등록 기능
...
- 지도 API에는 마커 표시 라이브러리를 사용하여 지도 상의 위치를 표시
